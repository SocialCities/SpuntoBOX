...
module.exports = {
    ...
    /**
     * Generate a token from input
     * @method token
     * @param {string}   input            the input to be hashed
     * @param {string}   type             type of the hashing algorithm
     * @return hash
     */
    token: function (input, type) {
        type = type || 'md5';
        var hash = Crypto.createHash(type).update(input).digest('hex');
        return hash;
    }
};